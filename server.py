...
def get_video_model_h5():
    """Load video classifier model (Keras .h5) using legacy tf-keras"""
    global _video_model_h5
    if _video_model_h5 is None:
        try:
            # Import tf_keras for legacy Keras 2 compatibility
            # This is required because the .h5 model was saved with Keras 2
            import tf_keras
            ...
            # tf_keras provides Keras 2 API compatibility with TensorFlow 2.20+
            _video_model_h5 = tf_keras.models.load_model(
                VIDEO_MODEL_H5_PATH,
                compile=False
            )
            logger.info("Video Keras model loaded successfully with tf-keras!")
